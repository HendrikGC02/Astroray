...
> ⚠️ **UPDATE 2026-07-25 last-call (architect wind-down — NEXT-RUN QUEUE; amends both banners below):** Overnight #2 closed with pkg141 (#518) and pkg154 (#521) merged; pkg149's CPU contract is FULLY met on draft PR #522 (furnace 0.997–0.999, peak 0.45°, azimuth root cause closed) but #522 is **merge-blocked by a low-roughness GPU-only furnace defect** (R=0.1 byte-unchanged 0.130, R=0.3 0.283 vs [0.90, 1.06]; the signed-off frontFace/TIR fix recovered only the roughness-scaling sub-lobe — R=0.6 0.896, R=1.0 1.0). **Next-run queue, in order: (1) pkg152** — promoted to head, scope widened to the whole GPU Disney twin divergence (metal dimness + the #522 low-roughness deficit, one instrumentation harness, split-clause in the spec); **(2) the #519/#520/#522 cascade** — merge in stack order after the owner approvals each PR's adjudication checklist names, #522 last and only after pkg152's fix re-greens its GPU furnace gate (measured, never pre-asserted); **(3) pkg150 re-baseline** once the corrected sampler is on main via #522; **(4) the pkg153 bisect arc** (root-cause + bisect plan committed as `ce85ad7`) — supervised or overnight per its spec's disposition rules. `disney.cpp` single-writer discipline and the one-HW-verifier-at-a-time rule carry over unchanged.
>
