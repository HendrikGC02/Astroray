...
**Manual actions:** restart Blender to load the restaged addon (volumes, GPU alpha shadows, viewport worker default ON, Nishita sky + sun disc, absolute IES). Owner decisions pending / vetoable: (1) **viewport worker default flipped ON** by the lead (pkg266; settle 13.5 / 15.0 ms vs 92 / 140 ms synchronous; storm 159 / 175 ms vs 230 / 211 ms — explained commit-cost residual) — veto with `ASTRORAY_VIEWPORT_WORKER=0` and say so; (2) the sun disc is now ~1.48× over Cycles on ground irradiance (#814) — physics-first says measure against the analytic E_sun before trusting Cycles' under-resolved disc; (3) pkg270 (blackbody spectral Planck) and pkg269 (GPU volumes) are next in the volumes track.
