...
**Owner decisions pending:** (1) pkg266 default flip of `ASTRORAY_VIEWPORT_WORKER` waits for the GUI Terra-4 Â§9 table (next session item 2). (2) pkg265 centre band 1.61Ã Cycles â #782 independent oracle decides. (3) pkg256 look: the bake has no sun disc and a warm Preetham horizon (see the contact sheet); accept as APPROXIMATED or prioritise #799. (4) CPU final-render sampler: authorise Sobol' for CPU F12 renders (#763 noise gap; pkg262 fork (b) left CPU on the random sampler)? (5) Spot IES: authorise a small addon package to export `ShaderNodeTexIES` (INTERNAL text) to the engine's existing IES hook (corpus SPOT dimmer). (6) Volumes: open a VDB/heterogeneous-media track now or after the Pillar 4 gate? (7) Glass: GPU walk (pkg265 P3) before or after the #782 oracle? Full list with context: `next-session-prompt-2026-09-11.md` Â§ Owner questions.
