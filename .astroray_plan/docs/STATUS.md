...
**Open PR #808 Batch E1** (#799 part 1 sun disc + exposure decomposition; #776 textured emitters in NEE/light tree): CI green, reviewers MERGE-WITH-FIXES (doc nits being applied); **HELD by the lead** — the sun-disc A/B contact sheet is over-exposed vs Cycles (blown deck, pink sky); absolute sunlit-ground ratio + direct:diffuse split requested before merge. Absolute-exposure finding: no single bake constant can meet #799 (2.2× drift across sun elevation) → engine-side spectral sky is the real Phase 2.
