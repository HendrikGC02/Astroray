...
**Last updated:** 2026-05-23 (Round 13 — pkg87d Cryptomatte acceptance gate in flight).

Wave summary 2026-05-23 (Round 13 start):
- **pkg87d Cryptomatte acceptance gate** (PR #347, in review) — name registry + manifest headers + IoU test harness + Python bindings. Psyop §3 `cryptomatte/<hash7>/{name,hash,conversion,manifest}` header emission via `writeExr()`. Test harness renders ground-truth isolation masks + reconstructs via Psyop matte-extraction algorithm; asserts IoU ≥ 0.95 per name. **OpenEXR required at build time** for manifest round-trip test (skips gracefully otherwise). Closes pkg87c deferred acceptance items.
...
| pkg55 | Wavefront SoA GPU refactor — Phase A.0 + A.1 (baseline + SoA scaffold). Phase B' (CPU-first restart) is authoritative on main. Sessions 2b/2c (oracles + skeleton); Sessions 3..8 (growing-oracle expansion: metal/dielectric/disney/thin_glass/diffuse_light/closure_graph); **Session N+1** (env-map miss + complete CPU pipeline). | **All CPU-only sessions DONE through N+1** (PR #327, 2026-05-21). Phases A.0/A.1/B' Sessions 2b/2c/3/4/5/6/7/8/N+1 complete; bit-identity gate PASS at every session (max abs diff 0.0). Production codegen byte-unchanged. **Sessions N+2..M (CUDA port of wavefront shade kernels) are now the top live track** — multi-session ~4 weeks; closes the viewport-parity acceptance gate. Two-tier gate definition landed PR #320. | A |
...
| pkg64-gpu | A | **Phase 1 done** | PR #323, 2026-05-21 — device `sms_attempt_device.cuh` + `GSphere.isCausticCaster` + scene_upload mirror + minimal probe harness. RTX 5070 Ti /verify: gate #2 (caster-flag round-trip) PASS, gate #3 (regression, 40 GPU tests) PASS. Gate #1 (CPU↔GPU rel-err ≤ 1e-3) spec-deferred — minimal probe is inert for that gate; needs full SMS Newton + refraction + visibility plumbing as Phase 1.1 follow-up. classify.py PARTIAL routing will surface if a regression appears. Phase 2 (megakernel integration) and Phase 3 follow. |
