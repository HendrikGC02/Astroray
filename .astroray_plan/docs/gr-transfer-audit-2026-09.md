...
- No bank rebaseline, external comparison claim, render-time attribution, or science-ready claim.
...
## Validation boundary
- The analytic tests establish self-consistency of the formula and prove that both live CPU disk callers share the production helper. They are not an independent external validation.
- The three live CPU GR dispatches (the two Renderer paths and registered `restir-di`) retain only finite/nonnegative samples. The Renderer paths then apply the normal user-selected direct/indirect contribution clamp; ReSTIR-DI has no corresponding user clamp mechanism. The former fixed 20 ceiling is removed. The disk's float storage rejects a nonrepresentable or overflowing addition while preserving the existing finite accumulated value; this is a numeric storage boundary, not a physical radiance cap.
...
- GYOTO is planned as a process-isolated GPL-3.0 executable oracle under the separately approved execution plan; no such run or match is claimed here.
