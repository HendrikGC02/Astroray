...
## Update 2026-05-29 — Chunks A–D-seed LANDED; D-radiance WIP (renders NOISE)

The full MNEE math+geometry foundation is **merged**, all unit-tested to ~1e-11:

| Chunk | What | PR |
|---|---|---|
| A | analytic half-vector constraint Jacobian (`half_vector_constraint.h`) | #387 |
| B | surface `(u,v)` partials + analytic Newton (`surface_partials.h`, `newton_iterate.h::solveAnalytic`) | #389 |
| C | multi-vertex block-tridiagonal chain (`manifold_chain.h`: `chainEval` a/b/c + `solveChain`) | #390 |
| D-seed | mesh seed-ray + chain converges on a triangulated prism (`mesh_caustic.h`: `seedChainFromRay`, `rayTriHit`, `makeFlatReproject`) | #391 |

Validated facts: chain Jacobian vs FD ~1e-11; damped block Newton converges in
3–4 iters on a forward-traced 2-refraction prism chain; seed-ray finds both
faces and converges. **Key gotcha (in-code):** seed vertices MUST use an
orthonormal `(u,v)` frame, not raw triangle edges — the Newton clamps unit-
tangent steps, so a non-unit parameterization makes the step-clamp meaningless
and diverges (raw edges → no convergence; orthonormal → 3 iters).

### D-radiance (WIP on branch `wip/pkg106-chunk-d-radiance`, NOT merged)

Wired the multi-vertex mesh MNEE into the live `sms_caustic_path_tracer`:
- `include/astroray/manifold/mesh_attempt.h` (new): `gatherTriangleCasters` +
  `runMeshSMSAttempt` (hero-IOR seed → chromatic `solveChain` → contribution).
- `plugins/integrators/sms_caustic_path_tracer.cpp`: gathers mesh casters in
  `beginFrame`; `sampleMeshSMSSpectral` adds the chromatic 2-vertex contribution
  in the `spectral_newton` path (gated on `!meshCasters_.empty()`).

**Render result (160², 256 spp, sms_caustic_path_tracer):** the pipeline works
end-to-end and a chromatic caustic appears — BUT it is **chromatic NOISE across
the frame, not a clean rainbow band** (the exact pkg104/pkg106 failure mode).
Hue *range* [0.04, 0.83] but that is random per-pixel speckle, not a band.

### Why it's noise — the precise remaining work (Chunk D-radiance + E)

1. **Geometry term too crude.** `runMeshSMSAttempt` uses a clamped solid-angle
   `G = cosX0·cosL/dist²`. The caustic only *localizes* with the generalized
   geometry term — Cycles `mnee_compute_transfer_matrix` (mnee.h lines 663–731):
   block-tridiagonal LU of the a/b/c blocks → `det_dh_dx` (∏ block determinants)
   and `dx1_dxlight`; `G = clamp(dw0_dx1 · dx1_dxlight, 2)`. Energy concentrates
   where the chain Jacobian is near-singular (the caustic) and is ~0 elsewhere.
   Port this (the chain already exposes the 2N×2N J via `chainEval`).
2. **Infinite-plane faces + no in-triangle validity.** The test prism faces are
   huge triangles, and the flat reproject never checks the converged vertex is
   *inside* the actual triangle → almost every receiver pixel finds a "valid"
   path → uniform noise. Use a realistic finite prism + reject vertices whose
   barycentric coords fall outside the triangle (or that reproject off-face).
3. **Visibility too permissive.** Tighten: each chain segment must hit the
   intended caster face (per-vertex probe, like Cycles mnee_path_contribution
   lines ~870-900) and the inside segment must stay in glass.
4. **Then Chunk E:** build the real triangulated equilateral prism + collimated
   sun + receiver in `benchmarks/reference_bank/.../prism-bk7-collimated`,
   render, measure `hue_spread` in the rainbow ROI, tune to ≥ 0.7.

Branch `wip/pkg106-chunk-d-radiance` has the WIP integrator + `mesh_attempt.h`
(renders the chromatic-noise caustic) as the starting point. Do NOT merge it
until it produces a clean localized rainbow band.
