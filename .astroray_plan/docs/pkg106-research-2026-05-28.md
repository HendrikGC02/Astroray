...
1. **Geometry term too crude.** `runMeshSMSAttempt` uses a clamped solid-angle
   ...
   `manifold_chain.h::chainEval` already exposes the 2N×2N J.
2. **Infinite-plane faces + no in-triangle validity.** The test prism faces are
   ...
   *inside* the actual triangle → almost every receiver pixel finds a "valid"
   path → uniform noise. Use a finite prism + reject off-triangle vertices.
3. **Visibility too permissive.** Each chain segment must hit the intended
   caster face (per-vertex probe, cf. Cycles `mnee_path_contribution` ~870-900).
4. **Then Chunk E:** build the triangulated equilateral prism + collimated sun +
   receiver in `benchmarks/reference_bank/.../prism-bk7-collimated`, render,
   measure `hue_spread` in the rainbow ROI, tune to ≥ 0.7.

Branch `wip/pkg106-chunk-d-radiance` has the WIP integrator + `mesh_attempt.h`
as the starting point. Do NOT merge it until it produces a clean localized
rainbow band. (This same section is also committed on that branch; on rebase,
dedupe to one copy.) Tracking issue: see GitHub issues (pkg106 rainbow finish).
