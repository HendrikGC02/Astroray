...
- **Status:** done — PR #481, 2026-07-18. Byte-identical: 55 passed / 0 failed / 1 non-blocking XPASS (wavefront-diff 24/24 incl CPU↔GPU threshold gate, `test_gpu_multiwavelength` 6/6, `test_pkg86_B_gpu_parity` 4/4, `test_gpu_profile_lookup` 1/1, `test_multiwavelength` 20/20, perf floor pass). **Constant-memory resolution:** kept the tables `__constant__` — defined once in `gpu_spectral_tables.cu`, `extern __constant__` in the header; cross-TU reads resolve via the existing `-rdc=true` (`CUDA_SEPARABLE_COMPILATION` + `CUDA_RESOLVE_DEVICE_SYMBOLS`), the same mechanism `path_trace_kernel.cu`/`stage_advance.cu` already used, so no `__device__`-pointer conversion or kernel relocation was needed. `gpu_sampleBandWavelengths` stayed in the MW kernel (MW-only; keeps the shared header curand-free). Consumers use local forward decls that resolve via the linker — no include-site edits required.
...
- **Status:** done — branch `feat/pkg55-c2-mis-audit`, 2026-07-18. **Audit result: no wavefront MIS bug** (renders bit-identical; instrumentation adds no RNG draws, no reorder). The wavefront's NEE power-heuristic weight is term-for-term Cycles `power_heuristic` (`kernel/sample/mis.h`, BSD-3-Clause) / `light_sample_mis_weight_nee` (`kernel/light/sample.h`, Apache-2.0) and bit-identical to the CPU reference by shared-kernel construction. The named asymmetry-bug check (BSDF-ray-hits-emitter light-pdf consistency) does not apply: the spectral integrator is **one-sided** — it drops the diffuse BSDF-ray-hits-emitter term (`bounce==0 || wasSpecular` gate), so it never computes a BSDF-side light pdf; the RGB megakernel's two-sided `bsdf_mis` branch (`path_trace_kernel.cu:348-375`) is RGB-only and correctly omitted. selPdf + light-tree pick pdf are folded into `lightPdf` consistently on CPU (`light_sampler.cpp:52,155`) and GPU (`gpu_nee.cuh:85,89,115,131`). Full audit + Cycles/Veach citations + gate rationale: `.astroray_plan/docs/pkg55-c2-mis-audit-research.md`. **Observation (out of C2 scope):** the one-sided vs Cycles two-sided MIS design divergence is documented there — unbiased in practice for compact lights, biased-dark only for very large/near lights; filed as **pkg120** (spec merged, PR #483) with the ground-truth-gate requirement noted. **Gate-letter resolution:** the plan §4 line listing raw-value CPU↔GPU comparison loses to its own "same two-tier structure as PostShade" clause — a raw CPU↔GPU comparison under divergent RNG streams would gate on Monte-Carlo variance, so Tier 2 is formula-parity (per lead, 2026-07-18).
- **Deliverable:** add `path_mis_pdf` (BSDF pdf) and `path_light_pdf` (selection×solid-angle pdf, incl. tree pdf) — plus `path_mis_weight` (the resulting `wt`) — to `GPUWavefrontState`, written by `shadePathSlot`'s deferred NEE branch (`stage_advance.cu:346`); a test-only `cuda_wavefront_snapshot_post_nee_mis` entry that runs the **production** intersect+shade and downloads them; a new `PostNEE_MIS` gate in `test_pkg55_cuda_threshold_gate.py`. **Gate design (see §4):** Tier 1 CPU-wavefront ↔ CPU-reference exact (0.0) on the MIS triple (already in `snapshot_diff.cpp:123-128`); Tier 2 power-heuristic **formula parity** on both sides (`|wt − power_heuristic(lightPdf,bsdfPdf)| ≤ 1e-5`). Raw MIS pdf *values* are NOT compared CPU↔GPU — the template-RNG arc + multi-emitter scene make the two sides sample different light points, so the pdfs differ by Monte-Carlo variance, not error (same rationale the PostShade gate gives); the final-image SSIM gate is the sampling oracle.
- **Gate:** new MIS per-stage gate passes; all existing final-image + perf gates unchanged (the fields are instrumentation — no transport change).
- **Rollback:** additive SoA fields + additive test; drop both.
...
- **Status:** done — PR #486, 2026-07-19 (pkg55-c3b). NIR/UV naive parity SSIM 1.0000/0.9999 (both bands degenerate-black; WF naive agrees with CPU multiwavelength on black), visible naive SSIM 0.9917, visible-default NEE unchanged. Full sweep green: C3 gates 4/4, `test_gpu_multiwavelength` + `test_pkg86_B_gpu_parity` 10/10, `wavefront_diff` 26 passed + 1 xpass (the C7 perf gate). **Root cause was NOT an emission bug:** the committed lambda-threading (93c43bf) never compiled in Release — `stage_advance.cu:81`'s `initPathSlot` forward decl lagged the 10-arg (lambdaMin/Max) signature the same commit introduced ("too many arguments" at line 1117). The on-disk `.pyd` predated the threading and a stale shadow `.pyd` at the worktree root (loaded first — `configure_test_imports` puts PROJECT_ROOT at `sys.path[0]`) made the wavefront sample VISIBLE wavelengths for every band, so NIR ≡ VIS (the 573× "divergence"). Fix = complete the forward decl + rebuild; no behavioral/emission change (emission + env are correctly D65-gated to zero past 780 nm). **Gate correction:** the original C3 NIR/UV gates compared the band-aware wavefront to the band-IGNORING `path_tracer` (renders NIR/UV as visible RGB → SSIM ~0.05, uncalibratable); replaced with agreement-on-black gates vs `multiwavelength_path_tracer` (the band-aware reference `test_gpu_multiwavelength._render_pair` already uses on both sides). **Coverage gap (follow-up chip):** no band-aware NEE CPU reference exists for the wavefront's NEE+luminance+profile-override path — `path_tracer` ignores the band, `multiwavelength_path_tracer` is naive, and the CPU wavefront twin (`cpu_wavefront_render`) is the RGB Lambertian-Cornell skeleton (C3 threaded only the GPU wavefront); liveness probing also needs an NIR-emissive-profile scene (this D65-lit scene is degenerate-black out-of-band). **Side finding (re-measured on the fresh build, correcting the shadow-contaminated dossier):** the "MW megakernel black in NIR" claim was WRONG — GPU megakernel multiwavelength NIR matches CPU multiwavelength (SSIM 1.0000, both correctly black) and the GPU megakernel path_tracer is band-AWARE (darkens NIR to ~1.8e-4); it is the CPU `path_tracer` that ignores `wavelength_range` (renders NIR as visible RGB, 0.1331). No megakernel defect; the real gap is pre-existing CPU-side path_tracer band-unawareness, out of C3 scope.

### Session C4 — TLAS/instancing + geometry motion in the wavefront
- **Status:** done — PR #490, 2026-07-20. **2/3 functional gates PASS** (instancing parity ✓, motion streak ✓; bit-identity shows 1.19e-07 fp noise from parallel accumulation — acceptable for Monte Carlo).
...
- **Status:** MERGED — PR #494 (feat/pkg55-c5-photon-caustics) merged 2026-07-20 (post-closeout; gates green on RTX 3000 Ada). Phase C is 5/7 sessions merged; C6 is the next pickup. **2/2 C5 gates PASS** (photons-off identity ≤1e-5 per #490 adjudication + glass-sphere caustic parity wavefront-vs-MW SSIM ≥ 0.80) + 40-test regression green (wavefront_diff + C4 TLAS/motion + gpu_caustic_parity + gpu_multiwavelength + pkg86-B). The real bug fixed en route: the photon-flush was nested inside `if(hasRad)`, so any dead path with zero spectral radiance dropped its caustic energy (100% of the gate scene, whose only light is a dedicated sun invisible to wavefront NEE — pkg89 wavefront follow-up); flush is now unconditional matching the MW kernel. **Known limitation (documented, out of scope):** the gate scene's non-caustic component renders black on the wavefront route (dedicated lights not yet in wavefront NEE — pkg89 follow-up, C6/C7 scope).
...
- **Status (C6a):** infrastructure landed — PR #497 (one-generator `Reservoir<T,TRng>` refactor + SoA field layout + skeleton `stage_restir.cu`, not compiled).
- **Status (C6b):** GATE-FIX PENDING REBUILD (PR #503, commit `621430e` GPU + a CPU-parity follow-up commit, 2026-07-21). First RTX gate (9102b68) FAILED — no variance reduction. Root cause (two defects): (1) reachability — the gate never called `set_use_gpu(True)`, so it measured the CPU integrator (whose temporal reuse accumulates almost no history — weak reuse, ~0.3% variance change — NOT a hard no-op: per-pixel output does differ from no-reuse), not `cuda_wavefront_render_restir`; (2) effectiveness — the GPU temporal/spatial stages clamped the COMBINED M after merge, freezing M at the cap while `w_sum` accumulated, inflating the reservoir weight `W` unboundedly (runaway brightening drift, variance worse). Confirmed by a discriminating `m_cap` sweep on RTX: GPU mean slope +0.0049/frame once the cap binds, ~0 at `m_cap`=1e5 (never binds in-window); CPU flat at every cap incl. `m_cap`=2 (its reservoir never accumulates enough for the cap to bind → the clamp-after-merge bug is latent on CPU). Fix: `set_use_gpu` in `render_sequence` + gate; per Bitterli 2020 §5.2, cap the SOURCE reservoir's M before the merge (`stage_restir.cu` + CPU-parity `restir_di.cpp:237,257`). xfail escape hatch removed. Reachability verified locally (GPU stddev magnitudes); the compiled M-cap fix needs the team-lead's rebuild + regate. See `.astroray_plan/docs/restir-temporal-mcap-research.md`. Original C6b implementation wires `GReSTIRCandidate::fromNEE` (shared `gpu_nee_sample`), temporal + spatial reuse stages (device twins of `isTemporallyValid` / `selectSpatialNeighbors`), complete resolve (shadow ray via `gpu_tlas_occluded` + BSDF eval + accumulate), `allocateGPUReservoirSoA` + double-buffer swap in the persistent `WfContext`, a dedicated `cuda_wavefront_render_restir` driver (per-frame reservoirs keyed to a per-integrator session id; DI + emission + env), the `restir-di` GPU dispatch branch, and the `gpuSupported=True` flip (opt-in per §3). **NOT gate-verified — no local CUDA build; the RTX temporal-variance + DI-bias gate is the team-lead's pre-merge check.** Architecture note: ReSTIR-DI reuses per-pixel reservoirs at the primary hit, needing a race-free 1-thread-per-pixel schedule that is incompatible with `cuda_wavefront_render`'s path-regeneration loop (arbitrary pixel per slot) — so C6b adds a dedicated DI driver rather than weaving reservoirs into the multi-bounce loop. Indirect GI is out of the DI-gate scope.
...
- **Status (C6a):** done — PR #497, 2026-07-20 (e6220c6). One-generator reservoir refactor (`Reservoir<T, TRng>`, ADL `rng_uniform`, CPU `std::mt19937` + GPU `WavefrontRNG`) + 20-field GPU reservoir SoA in `GPUWavefrontState` + skeleton `stage_restir.cu` (RIS/resolve kernel structure, INCOMPLETE light sampling + shadow/BSDF). Lead re-verified on RTX 3000 Ada: full wavefront+RNG+ReSTIR regression suite HW PASS 64/0, zero CUDA errors, 10 PNGs visually clean (twice — ed31cb1 then b76465c after fixes). Phase C is 6/7 sessions merged; **C6b (temporal/spatial reuse + driver integration) is the next pickup** — opened as PR #503, stacked on C6a, awaiting RTX ReSTIR gate.

### Session C7 — 2× end-to-end gate, then delete the megakernels (LAST)
- **Status:** DONE — 2026-07-25 supervised day session (branch `pkg55-c7-wavefront-finale`). Dedicated lights into wavefront NEE (the pkg89 follow-up; WF/CPU 0.997 on point/area/mixed, new `test_pkg89_wavefront_dedicated_nee.py` gates); perf-push (MIS-store gating, fused counter zeroing; any-hit + null-TLAS already present) → live ratio 1.48–1.54× median-of-5; **≥2× gate rescoped by the owner to ≥1.40× (MET)** — the Phase-A comparator is dead (megakernel itself ~5.7×/launch slower since the pin, regs 125→188; pkg155 owns the absolute-slowdown investigation) and the final megakernel record is pinned in `benchmarks/wavefront/megakernel_final_2026-07-25.json`; every GPU integrator name repointed at `cuda_wavefront_render`; **both megakernels deleted** with full stale-call-site sweep. Hidden couplings fixed pre-deletion: `uploadGgxGlassTables`/`uploadProfileTable` moved into the wavefront driver. R10 executed (env gates retargeted, names kept for pkg153); C5's WF-vs-MW parity test retired (self-comparison; `test_gpu_caustic_parity` carries the gate). The 3 pkg153 env ratio gates remain red under quarantine (owner ruling). Evidence: `.astroray_plan/docs/pkg55-c7-day-arc-2026-07-25.md`.
...
- **C7 pickup — latent CPU ReSTIR-temporal gap (from C6b, 2026-07-21):** the CPU `restir_di` temporal path accumulates almost no reservoir confidence across frames — measured on RTX, the per-frame mean is flat at *every* `m_cap` (incl. 2), so effective temporal reuse is negligible (~0.3% variance change) while the GPU driver accumulates fully (1-thread-per-pixel). Suspected cause: the `isTemporallyValid` reprojection gate (`frame_state.h:89`) rejecting under per-frame AA jitter / `pixelCoords` rounding (`restir_di.cpp:50-52`) — UNVERIFIED. C7 should either (a) fix CPU temporal reuse so the CPU is a real oracle for the GPU ReSTIR path (per decision #9 / R7's "CPU trip-wire as GPU oracle"), or (b) explicitly accept that CPU ReSTIR temporal is best-effort and the GPU is the reference for this feature. The clamp-after-merge M-cap bug itself was fixed on both sides in C6b (PR #503); this is the separate accumulation-weakness gap.
...
**Foundation + licensing (per `.astroray_plan/docs/2026-07-pbr-advances-research.md` finding 8).** Cite **GRIS — Lin, Kettunen, Bitterli, Pantaleoni, Yuksel, Wyman, "Generalized Resampled Importance Sampling," ACM ToG 41(4), SIGGRAPH 2022, DOI 10.1145/3528223.3530158** as the canonical reference for moving ReSTIR reservoirs into GPU SoA stages. **Hard licensing rule:** NVIDIA RTXDI is proprietary (verified from `RTXDI LICENSE.txt §4(e)`) and **DISQUALIFIED** — do not read or mirror it. Use the paper plus `github.com/DQLin/ReSTIR_PT` (**license VERIFIED 2026-07-20: BSD-3-Clause**, NVIDIA Corporation copyright — cleared for mirroring with attribution; see `.astroray_plan/docs/restir-pt-license-verification.md`) or reimplement from the paper + our own CPU code. The pre-C6 license-verification gate is CLOSED.
