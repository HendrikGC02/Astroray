...
**Status:** done (PRs #407+#410, 2026-05-31 — harness complete + CPU/cross-engine acceptance both closed). PR #407 added 3 tests (`tests/test_reference_bank_smoke.py`) closing CPU output-verifiable acceptance: real runner vs real references, broken-render gate-fails, prism `hue_spread` 0.753 ≥ 0.7, Schwarzschild `dark_disk` 0.053 ≥ 0.03. PR #410 re-rendered the `disney-sweep-cycles-compared` cross-engine Cycles reference (Blender 5.1) with the PR #405 FOV fix → SSIM 0.7611 ≥ 0.65. Phase-2b astrophysics scenes (ADAF/jet) stay un-gated — owner tuning session. Scene set decided 2026-05-30 (`.astroray_plan/docs/visual-reference-bank-design-2026-05-30.md`).
