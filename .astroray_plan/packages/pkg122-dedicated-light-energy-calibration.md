...
**Status:** done (PR #500 merged 2026-07-21 as `2b18a1d` — Defects 1–3 + DISTANT irradiance re-derived vs Cycles `kernel/light/{area,point,spot,distant}.h`; GPU==CPU 0.997–0.998 all types; gross oracle factors 0.13×/3.6×/14× ELIMINATED; owner accepted the G4 spot gate-floor relaxation. The squash includes the caustic-scene DISTANT-units recalibration (`c32eda8`) — branch `pkg122-light-energy-calibration` is fully merged, no PR pending. **Defect 4 adjudicated in pkg142/#512: keep `RGBIlluminant` (D65), zero code change.** The residual ~7–16%-vs-Cycles offset is re-attributed to **pkg146** — the `Astroray-pkg122` worktree is kept intentionally as oracle evidence for it.)
