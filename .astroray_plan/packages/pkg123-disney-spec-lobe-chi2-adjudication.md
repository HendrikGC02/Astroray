...
**Status:** done (PR #498 MERGED 2026-07-21 as `587b554` — chi² failures 163 → 0 over 5 rounds; follow-ups pkg144/pkg145 dispatchable, pkg138/pkg141/pkg124 unblocked). 2026-07-20: epsilon-contaminated pdf denominators adjudicated + Disney specular-lobe chi² gates un-xfailed. 2026-07-21 render-regression round (architect verdict: ship the physically-correct uncapped true-D eval): `5e2080c` removed the dormant `eval()` firefly cap the D-fix woke (restored near-delta metal reflection); `a416287` fixed the reflectance test's own measurement (uniform→importance-sampled `rho()`, pbrt-v4 §14.1.6 — the uniform integrator false-positived metallic at 1.31 / false-negatived grazing); `602acd4` quarantined the true 15-config grazing-dielectric energy leak (per-config bounds, →pkg145, tight 1.02 elsewhere) + xfail'd the 2 highlight tests (→pkg144, always-on sLum>20 direct clamp, intensity-proxy-confirmed). Final CPU sweep 0 failed: energy 271p, chi² 7p/1xf, reflection 1p, material 19p/2xf. Follow-ups: pkg143 (clearcoat, blocked on #498), pkg144 (firefly clamp), pkg145 (specular energy refit, spec re-anchored to the enumerated grazing set).
