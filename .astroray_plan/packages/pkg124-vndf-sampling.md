...
**Status:** open — dispatchable (**UNBLOCKED 2026-07-23**: pkg123/PR #498 merged 2026-07-21 as `587b554` with the chi² gates un-xfailed — the swap now measures against a green baseline. Coordinate with pkg138/pkg145: all three edit `disney.cpp` sample/pdf regions; sequence merges). **2026-07-24 overnight: do NOT dispatch — serialized BEHIND the Lane-A chain (pkg151 → pkg149 → pkg150).** Two reasons: (a) same `disney.cpp` sample()/pdf() region as the chain — parallel work guarantees conflicts; (b) **pkg149's `670e583` fixes an azimuth swap inside `sampleGgxVNDF` itself** — the exact helper this package promotes to the reflection lobe. Landing pkg124 on the pre-fix helper would propagate the swapped azimuth to a second lobe and then collide with the chain's rebase. After the chain merges: re-baseline the chi² grid + the `disney.cpp` line anchors (they will have shifted), then this package is the natural next `disney.cpp` pickup.
