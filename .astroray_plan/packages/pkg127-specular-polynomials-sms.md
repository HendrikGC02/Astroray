...
**Status:** done — Triage 2026-09-12 (half-implemented audit, `half-implemented-triage-2026-09-12.md`): Phase 1 landed (#685, `sms_specular_poly` default OFF); Phases 2/3 live in pkg227. Was: Phase 1 LANDED (#685, 2026-09-04) — deterministic single-bounce sphere specular-polynomial seed finding, CPU, both integrators, behind `sms_specular_poly` (default OFF). Sphere-specialized exact degree-6 solver; physically-correct MNEE weight; glass-sphere reference re-blessed (owner decision); a latent default-path bug surfaced + filed as pkg226. Phases 2 (two-bounce; prism-triangle scope fork) + 3 (GPU mirror) OPEN. Spec DETAILED 2026-09-03 from a web-verified research note (`.astroray_plan/docs/pkg127-specular-polynomials-research.md`). The highest-value *bounded* caustics-quality upgrade on the roadmap.
