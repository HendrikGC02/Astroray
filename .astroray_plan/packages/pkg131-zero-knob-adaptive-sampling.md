...
**Status:** BLOCKED, prerequisite IN PROGRESS — the hard prerequisite (a progressive/low-discrepancy RNG with the Sobol/PMJ prefix property; the wavefront RNG is PCG32 white-noise) is now being built as **pkg224** (`packages/pkg224-progressive-sampler.md`, owner-confirmed 2026-08-29: hash-Owen Sobol' / opt-in runtime flag / GPU-only first). **Unblock pkg131 as soon as pkg224 lands** — verify pkg224's opt-in progressive mode is the one pkg131's convergence scheduler drives (the prefix property is exactly what adaptive per-pixel sample counts need). Spec-filing PR #492; memory `pkg131-blocked-on-progressive-sampler`.
