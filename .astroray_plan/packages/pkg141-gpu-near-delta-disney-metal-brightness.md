...
**Status:** open — dispatchable (**UNBLOCKED 2026-07-23**: pkg123/PR #498 merged 2026-07-21 as `587b554`; the CPU baseline is canonical on `main` now — re-anchor any branch-pinned evidence rows to `main`). **2026-07-24 overnight: Lane B (parallel with the Lane-A disney.cpp chain).** Parallel-safety notes: (a) this package must NOT edit `plugins/materials/disney.cpp` (the CPU side is canonical and is Lane A's exclusive file — spec Non-goals already say so); (b) its edits land in `include/astroray/gpu_materials.h` (metal/inline-sampler region ~849-857) and/or the closure-graph metal lobe, while pkg151's GPU mirror touches the closure-graph *dielectric* region of the same header — different lobes, but **serialize the merges and rebase whichever lands second**; (c) RTX hardware verification must not run concurrently with Lane A's (memory `cuda_verifier_concurrency`).
