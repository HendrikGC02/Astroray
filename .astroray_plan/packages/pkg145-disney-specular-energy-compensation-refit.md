...
**Status:** done (PR #513 merged 2026-07-23) — diffuse-under- specular Cycles `closure_layering_weight`/OpenPBR coupling added to `plugins/materials/disney.cpp::eval()`; full 90-config x 3-angle energy grid 1.2048 -> 1.004 (N=65536); pkg143/clearcoat fork resolved by measurement (kept the original pkg60 clearcoat mechanism, unchanged — the "preferred" table_ggx_E route regressed on the Dr/Gr fixed-alpha mismatch); quarantine in `test_disney_energy_conservation.py` retired. GPU parity N/A (`gpu_disney_ eval` carries no CPU compensation twin at all, pre-existing). Head SHA d4eecbe04a348a7107aac5a7e519ff3ae5fe2085; HW visual gate still pending.
**Depends on:** TBD
