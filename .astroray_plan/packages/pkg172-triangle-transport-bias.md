...
**Track:** A (RTX-gated)
**Status:** done — effect (A) (PRs #551/#553/#576, 2026-08-07 → 2026-08-10 — pbrt-v4 guarded-pdf form (`pdf > 1e-8f ? 1/pdf : 0`) replaces the biased `f/(pdf+1e-3)` estimator, closing the universal 0.628%/bounce (`2π·ε`) energy loss: CPU path_tracer/wavefront/multiwavelength (#551, 15 sites, clearcoat MSE floor re-pinned 5e-5→3.5e-5 with in-test justification), `neural_cache`/`restir_di` follow-up legs (#553), and the two remaining GPU NEE light-pdf sites in `stage_advance.cu`/`gpu_nee.cuh` (#576, register-neutral, no firefly-gate shift). Effect (B) was TRANSFERRED to **pkg173** (bounce-1 geometry-sampling parity) 2026-08-02 and is tracked there, not here — pkg173 remains open, below the Integration Milestone per the owner's 2026-08-03 sequencing.
