...
**Status:** Stage 0 done (owner-review artifact — mapping table `docs/blender_parity/pkg176_stage0_mapping.md` + machine-readable `blender_addon/settings_map.py`; 58 direct / 4 approximated / 19 dropped / 9 astroray-only across 90 settings-level rows; Route-2 §6 five hard rules honoured). Stage 1 settings-plumbing implemented (PR #555, 2026-08-08 — exporter reads the 14 DIRECT-mapped native props via `blender_addon/native_settings.py`; custom duplicates are deprecated aliases with one-release back-compat + per-render migration note; 21 new unit tests green; on-hardware pkg175 smoke/render verification DEFERRED to lead). Stage 2 panel adoption implemented (PR #556, 2026-08-08 — adopted the native Cycles Light Paths panels (`CYCLES_RENDER_PT_light_paths` + `_max_bounces`/`_clamping`/`_caustics`) for `CUSTOM_RAYTRACER` via an explicit checked-in `ADOPTED_NATIVE_PANELS` list with a fail-loud register guard; per-panel own-copy of the shared Cycles `COMPAT_ENGINES` set to avoid leaking onto un-honest panels; retired the redundant custom Light Paths panel; sampling panels deliberately NOT adopted — Cycles bundles `samples` with `dropped` adaptive/time_limit + `approximated` use_light_tree, so no sampling panel is fully honest; 5 new + 112 addon-regression tests green; on-hardware panel/visual verification DEFERRED to lead). Stage 3 world/light/camera completion implemented (PR #TBD, 2026-08-08 — verified against the owner-ratified Stage-0 table AND the live engine bindings that EVERY DROPPED-SILENT world/light/camera cell needs new engine capability (camera `setup_camera` has no ortho/clip params; light adders have no `specular_factor`), so zero cells are direct-mappable today and none were wired below the boundary; instead added `native_settings.report_unsupported_native_controls` — a bpy-facing, engine-untouching translator check that emits ONE consolidated per-render WARNING naming the DROPPED controls the user actually set to a render-affecting value (camera ORTHO/PANO projection, changed near/far clip, polygonal/anamorphic bokeh under DoF, per-light `specular_factor`) so the steering wheel is never silent; untouched defaults stay quiet; `show_cone` deliberately not warned (viewport-only gizmo, never render-relevant); follow-up engine-capability packages enumerated in the PR; 12 new + 65 addon-regression tests green; on-hardware pkg175 smoke/render verification DEFERRED to lead). Stage 4 open — staged, expect multiple PRs
