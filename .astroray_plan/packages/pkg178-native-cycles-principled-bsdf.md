...
**Status:** done (PRs #566–#581, 2026-08-07 → 2026-08-10 — native Cycles-Principled BSDF COMPLETE, Stages 0–5, CPU+GPU byte-mirrored throughout). Stage 0+1 core-lobe scaffold + Stage 2 GPU closure-graph twin (#566/#567); Stage 3 advanced layers — coat/sheen (Zeltner 2022 LTC)/approx-SSS/emission (#571) behind `template<bool HasPrincipled>` D4 shade-path isolation (#570); CPU UV-aligned tangent plumbing (#572); height-correlated Smith G2 for the reflect lobes (#573); anisotropic GGX (αx≠αy) + gated GPU UV tangent (#574); alpha as a delta transparent lobe (#575); Stage 4 thin film + thin wall (Belcour-Barla: shared utility + CPU dielectric #577, CPU conductor #578, GPU twin #579, thin wall + thin subsurface #580); Stage 5 Blender native routing incl. thin-film sockets, `use_native_principled` default ON experimental (#581). Non-principled `<false>` shade-path STACK held at 3608 B across every stage (D4 isolation holds); a pre-existing `ggxReflect` eval-D/pdf-D low-roughness mismatch surfaced during Stage 4 and was fixed separately as **pkg182** (PR #582). Production default-flip (native vs Disney-conversion) is gated on the full Blender parity matrix on hardware, per the owner's auto-flip authorization (memory `pkg178-repin-and-stage5-autonomy`).
