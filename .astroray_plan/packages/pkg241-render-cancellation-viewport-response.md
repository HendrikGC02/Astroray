...
- [ ] 2026-09-08 — **P2.2 (design §12 items 1–5) code delivered** (`feat/pkg241-phase2-p22`,
  ...
  - **GUI measurement (both scenes, worker ON/OFF, continuous + settle) + present_check + visual
    inspection: PENDING** an OpenMP-OFF addon build under the GPU lock (another lane's CUDA build was
    active at code time; a second sm_120 build would risk the concurrent-nvcc silent-death mode).
