...
- [ ] 2026-09-08 — Phase 2 design **Revision 3** written: Terra review 2 (call 1/4) returned **BLOCK** with seven ordered corrections; items 1-7 resolved in `pkg241-phase2-offthread-design-2026-09-08.md` §3.5/§3.2 (global admission token over every renderer mutation), §3.2/§3.3 (snapshot commit point + full fields + epoch discard rules), §3.4 (submit latest `desired_generation`, validate every notification, F12/teardown do wait), §3.5 (F12 process-wide pause gate), §3.6 (central `stop_all()` + `atexit` + strong-ref quarantine), §3.8 (settled denoise over the accumulated buffer -- fork with recommendation), §3.7/§9 (bounded GIL range before NumPy packaging + null-callback identity test + spike present-chain/device/comparator additions); tests 11-17 added to §5; verbatim review pasted as §10. Awaiting Terra re-review, then the A2 spike.
- [ ] 2026-09-08 — Phase 2 design Revision 2 written per Terra/lead/owner; awaiting Terra call; spike next.
