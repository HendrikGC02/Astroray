...
**Pillar:** 5
**Track:** A
**Status:** in-progress — #737 follow-ups landed (PR #809, 2026-09-13: scale-relative singular check + valueOffset 3-D bump gradient, 6 tests + GPU twins green); Phase 2 real-Blender parity still open. Was: in-progress — Triage 2026-09-12 (half-implemented audit, `half-implemented-triage-2026-09-12.md`): finish in a batch — Phase 2 real-Blender parity plus the two #737 follow-ups. Was: in-progress — Phase 1 in review (PR #737, 2026-09-07; CPU analytic oracles green, GPU parity pending lead build); Phase 2 real-Blender parity still open
...
- [x] 2026-09-07 — Phase 1 transformed-p contract LANDED on main in commit `fe1c95c` (lead error: swept into a docs commit while the PR #737 cherry-pick was applied for GPU verification; the code is the PR #737 code, GPU-verified 08:45 — 16 contract tests incl. 3 GPU twins, 129 procedural regressions, cpp-abi-guard PASS). PR #737 then carries only the bit-exact bake-dedup key fix.
- [x] 2026-09-13 (batch-I) — pre-review follow-ups from PR #737 LANDED (CPU): (a) the singular-Mapping check is now SCALE-RELATIVE (`Texture::mappingIsSingular`, `include/advanced_features.h`): `|det| < 1e-6 * (||c0|| ||c1|| ||c2||)` — the normalized determinant in [0,1], which is 1 for an orthogonal frame at ANY scale, so a legitimate tiny uniform scale (det=s³, e.g. 0.001→1e-9 which the old absolute 1e-8 test falsely flagged) is no longer reported singular, while a genuinely rank-deficient frame (zero/parallel column) still is. (b) `Texture::valueOffset` Mapping branch now perturbs the mapped 3-D point by `(du,dv,0)` in addition to the 2-D sample coord, so p-reading procedurals (Noise/Wave/Musgrave/Checker via the HitRecord overload) get a NONZERO bump gradient under a Mapping (was identically zero). Byte-identical for IMAGE bump (uses only the 2-D coord), so CPU/GPU image-bump parity (stage_advance.cu post-mapping offset) is preserved; GPU has no procedural-bump path to diverge from. Tests: `tests/test_pkg242_followups_737.py` (6 CPU: noise/checker gradient nonzero under Mapping; tiny-uniform-scale not singular; rank-deficient singular). 15 passed / 3 GPU-skipped incl. the Phase-1 contract suite; 46 passed texture/mapping/bump regression; 16 passed bump/displacement. CPU-only build (`build_cpu`, CUDA OFF). GPU twins + the `.def` bindings (`texture_value_and_offset`, `named_texture_mapping_singular`) await the lead's RTX CUDA build (blocked behind the shared GPU lock this session).
