...
**Pillar:** 5
**Track:** A
**Status:** in-progress — CPU leg merged (#747); GPU wavefront leg PR feat/pkg258-gpu-2026-09-08 (2026-09-08). GPU env NEE live: sun-disc GPU RMSE ratio **0.520** (gate ≤0.58), furnace GPU **0.9946**, shade REG **254** unchanged (STACK max 8736 vs 8600, +136 B), all CPU/GPU parity + bit-identity + snapshot gates green. Residuals (not blocking correctness, deferred to a follow-up): `hdri_exterior_hair` CPU-vs-GPU harness (needs an OpenMP-off addon build) and metal_sweep GPU frame-time ±5% bench not run this session.
