...
**Pillar:** 5
**Track:** A
**Status:** done — PR #812 merged 2026-09-13: present-rate=0 root-caused to two bugs (spurious view_update over-cancel; orphaned worker holding the global admission token) and fixed; chunk-spp column; §9 gate table on the RTX 5070 Ti (settle: tick-gap p95 13.5 / 15.0 ms, present-rate 1.0, cancel p99 47 / 71 ms — all PASS; storm 159 / 175 ms = explained main-thread commit-cost residual vs 230 / 211 ms synchronous). Worker default flipped ON (8daa6797) then REVERTED to opt-in (owner 2026-09-14, PR fix/viewport-worker-opt-in): in the real interactive viewport the worker does not present while the camera is orbited and refinement can stick at the reduced first-unit resolution — the driver never exercised viewport navigation. Follow-ups: (a) fix those two behaviours + a reproduction through real navigation on the isolated GUI, (b) reduce the coalesced-commit cost so the storm row meets 33 ms; only then re-propose the default. **Follow-up (a) fixed — Batch M PR (#817), branch `feat/batch-m-viewport-817`:** present-while-orbiting (a camera-only move over a cheap reduced preview no longer cancels it and the mailbox presents any frame at/above `present_floor_generation`, which only a scene/material edit raises) + reduced first unit capped to one coarse chunk before the full-res refinement. Proven by a deterministic bpy-free reproduction driving the REAL `_ViewportSpikeWorker` (orbit presents 0→continuous; reduced presents 16→1 then full res) in `tests/test_pkg266_orbit_present.py`; a `--ui-pattern orbit` §9 row + reducer resolution metrics added (`tests/test_pkg266_orbit_reducer.py`). GUI §9 re-run worker ON/OFF (both scenes + storm) and the #721 storm per-phase measurement are deferred to the lead's GPU closeout — the isolated launcher installs the addon into the user profile, which would disrupt the owner's live Blender on 9876 mid-session. Default stays opt-in. Was: in-progress — native bounded dispatch + global admission token + coalesced dirty-domain commit landed on `feat/pkg266-viewport-p23-bounded-dispatch` (2026-09-10); GPU build + Terra-4 GUI re-measure in the same lane (lead flips to done on the gate table)
...
- [~] 2026-09-13 — **Batch G (PR pending): present-rate=0 root-caused + fixed; §9 settle gates all PASS (present-rate gradeable 1.0).** Two independent causes of the Batch B UNGRADEABLE present-rate: (1) `_worker_view_update` called `worker.request()` on EVERY invocation, but Blender re-fires `view_update` during a settle span (depsgraph re-eval on the worker's own `tag_redraw`) with empty/selection-only `updates` → every in-flight render was cancelled before `render_end` → no `terminal_publication`. Fixed by a cache-free `_depsgraph_has_image_changing_update` guard (mirrors the synchronous path's no-domain early return). (2) an **orphaned viewport worker held the process-global admission token**: the Exporter strong-refs its engine and `_LIVE_VIEWPORT_SESSIONS` strong-refs the Exporter, so a superseded RenderEngine is pinned alive (`__del__` never fires) and its worker keeps rendering + holding the token, starving the live worker (confirmed in-GUI: two distinct worker instances). Fixed by `_reap_dead_viewport_sessions()` (drains freed-engine sessions via `ReferenceError` on `as_pointer()`, keeps live siblings §3.5) + an engine `__del__`. Plus the §9 settled target is pinned to 64 spp in the recorder (scenes ship preview_samples=1024, unreachable in a settle span). §9 re-measure (idle GPU, isolated Blender 5.2 @9877, Terra-4, 2 reps): **worker-ON SETTLE passes every gate on both scenes** — tick-gap p95 **13.5 / 15.0 ms** (vs 92 / 140 ms synchronous; Cycles 6.6 ms), present-rate **1.0** (completed 2 / 3, gradeable), cancel_pump p99 47 / 71 ms, frame_age ≥ 0, mailbox ≤ 1, 0 CUDA errors, chunk_spp measured (max 64), 14.8 spp/s. **Continuous STORM tick-gap p95 159 / 175 ms FAILS** (explained: main-thread commit ~150 ms/edit — the coalesced-commit residual, not render blocking). Tests: `test_pkg266_present_rate_settle.py` (+ reaper test), `test_pkg266_dirty_domain_commit.py` 16, `test_pkg241_spike_worker.py` 10 green. Item 2: presented-chunk spp threaded → measured samples/s + chunk_spp column. **Worker default FLIPPED ON (lead decision 2026-09-13):** the worker beats the synchronous path on every §9 row on both scenes (settle 13.5 / 15.0 ms vs 92.3 / 140.2 ms; storm 159 / 175 ms vs 230 / 211 ms), and the storm criterion is "≤ 33 ms OR an explained residual" (the ~150 ms/edit main-thread commit). `ASTRORAY_VIEWPORT_WORKER` still forces the synchronous fallback OFF (0/false/off/no). Follow-up (non-blocking): reduce the main-thread commit cost so the storm tick-gap also meets 33 ms. Table: `benchmarks/viewport_parity/results/2026-09-13-batchG/SUMMARY.md`.
