...
3. **Sessions 3..N — Growing-oracle expansion.** As each new shade kernel (metal, dielectric, disney, thin_glass, diffuse_light, closure_graph) is added to the CPU wavefront, both reference PTs grow alongside to cover the same feature surface. Trip-wire test scene grows; equivalence test scene grows. The reference PTs are "growing oracles" — they always match the current CPU wavefront feature surface; never lead, never lag.
   ...
   - **Session 4 — Dielectric (open — PR #308, 2026-05-16).** Scope guard extended to `lambertian + metal + dielectric`. Test scene `dielectric_cornell.py` adds one glass sphere (IOR 1.5) alongside metal + Lambertian. Shared kernel's virtual dispatch already handles dielectric correctly via `DielectricPlugin`. Bit-identity gate: pending CI verification (expected PASS by construction; max abs diff exactly 0.0, diverging fields = 0). Production codegen: byte-unchanged (only `src/cpu/wavefront/` modified).
