...
- [ ] **Empty-hook walltime overhead (gate ≤ 5%):** cornell parity scene, same as Phase 2. Infrastructure present. *2026-06-12: measurement changed from pinned-absolute-walltime to toggle-relative (caustics on vs off, interleaved in the same run), mirroring the CPU pkg64-3 cost gate this gate is sourced from — the pinned-seconds baseline was thermal/load-sensitive and false-failed 3× under sustained GPU load while passing in isolation. Walltime baseline `.npy` files removed; no RTX baseline-pinning needed for this gate.*
