...
## Deliverables (independent sign-off gate)
...
**(b) Independent Claude sign-off on the proposed fix** — once a fix is drafted
(by the routed `package-implementer`), **before it is pushed for re-gate**,
invoke the `sign-off` agent to run an independent `claude -p` review of the
proposed diff against the root cause you identified. The sign-off must return
one of:
...
drafted the fix (open-weight draft → Claude sign-off). The `sign-off` agent's
reviewer prompt is **adversarial by mandate**:
