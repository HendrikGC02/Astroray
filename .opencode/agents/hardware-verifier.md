...
as a single command so the MSVC env is live for the build:
...
If you cannot read the PNGs yourself (no vision on the current model), invoke
the `sign-off` agent to run a `claude -p` visual inspection of each render
against its reference instead — do not skip the visual step.
