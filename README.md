...
| **Light transport** | Path tracing with NEE + MIS, Russian roulette, adaptive sampling. Spectral path tracer with Jakob-Hanika RGB→spectrum upsampling and CIE 1931 2° CMFs. Specular Manifold Sampling for spectral caustics. ReSTIR DI and Neural Radiance Caching are wired as plugin integrators (CPU only today). |
| **Materials** | Native Cycles Principled (`principled`, pkg178), Disney BRDF with Kulla-Conty energy compensation tables (pkg60), Lambertian, Metal, Dielectric with Sellmeier dispersion, Subsurface, Emissive, Volumetric (heterogeneous NanoVDB grids with per-wavelength extinction; Principled Volume emission incl. a spectral Planck blackbody on both backends). CPU + GPU material capability metadata; no silent fallbacks. |
...
| **GPU** | CUDA **wavefront path tracer** (pkg55) — staged intersect/shade with material-sorted buckets, path regeneration, dedicated NEE shadow stage, any-hit shadow rays — measured **1.50× faster** than the previous megakernel on the 7-material gate scene, at per-channel image parity. Two-level BVH (TLAS/BLAS) instancing with transform-only refit (pkg114), GPU light tree for many-light scenes (pkg86-B), deformation motion blur (pkg88-C.0), multi-wavelength spectral rendering with measured CPU/GPU parity (pkg54 chain), and a heterogeneous-volume stage (NanoVDB device grid, at most 8 media, with constant and blackbody volume emission, #828). |
| **Blender** | Blender 5.2 addon: persistent viewport sessions (pkg52), incremental scene sync (pkg56), native shader nodes (pkg57), and automatic GPU instancing (pkg114). Historical pkg81 A/B measured steady-state pan-frame p99 at 0.84× Cycles-OPTIX; this does not establish the pending camera/material-edit and cancellation contract. Cycles-derived procedural evaluators (pkg115) and bounded affine image/program mapping (pkg230b) are available, and procedural textures (Noise, Checker, Wave, Gradient, Voronoi, Magic) now work as inputs to Math / Mix / Color Ramp / Map Range chains on both backends (#821) — baked on GPU (64×64 for UV, 64³ for Generated/Object) with a base-colour fallback warning when a program has more than two texture inputs (#826, #844); coordinate-side non-affine math remains unsupported (#822), and procedural transformed-coordinate/bake parity (pkg242) and mapped normal/bump provenance (pkg245) remain open. An opt-in off-main-thread viewport worker (`ASTRORAY_VIEWPORT_WORKER=1`, #819) presents a reduced preview while orbiting and refines at full resolution; material, light and geometry edits trigger a full scene re-sync (~125–132 ms per edit; the incremental replay rendered stale data, #850). |
