# -*- coding: utf-8 -*-
"""pkg129 A/B — single (config, engine, device) render leg (runs INSIDE Blender).

Spawned once per leg by ``harness.py`` for subprocess isolation (pkg71/pkg119b
discipline: Cycles and the Astroray addon hold conflicting global state, so each
renders in its own Blender process). Builds one metal-sweep config via
``scenes.build_metal_scene``, renders it with one engine+device, and writes the
LINEAR scene-referred pixels to ``<out>.npy`` (float32 HxWx3) plus an sRGB PNG
for the human report.

The astroray-addon bootstrap and the linear EXR->npy plumbing are adapted from
``benchmarks/blender_parity/render_leg.py`` (pkg119 Phase B) — the same
OpenMP-OFF-.pyd / ASTRORAY_PYD_DIR pattern (memory: mingw_openmp_blender_deadlock
requires the addon build, not build_cuda, when astroray is imported inside
Blender). This file is a rough-metal-specific sibling, not a modification of that
lane's harness.

Contract with the driver:
  * On success prints ``PKG129_LEG PASS`` and writes ``<out>.npy``.
  * On ANY failure prints ``PKG129_LEG FAIL <reason>`` and exits 0 (Blender
...
Run:
    blender --background --factory-startup --python render_leg.py -- \
        --config metal_chromatic_r030 --engine CYCLES \
        --out <stem> --res 128 --samples 256
"""
...
def _fail(reason: str):
    ...
    sys.exit(0)  # driver keys on the sentinel, not the exit code


def _bootstrap_astroray_addon(repo_root: Path):
    """Load the OpenMP-OFF .pyd + register the addon (mirrors blender_parity)."""
    ...
    print(f"[pkg129-leg] astroray module: {astroray.__file__}", flush=True)
