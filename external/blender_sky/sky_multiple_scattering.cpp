...
/* -------------------------------------------------------------------------
 * Astroray addition (Batch J, 2026-09-13) -- NOT upstream. A context that
 * precomputes the transmittance LUT once and evaluates multiple-scattering sky
 * radiance (CIE XYZ) for arbitrary Z-up view directions, so the equirect bake
 * can evaluate the model per output pixel directly (exact orientation, no LUT
 * resample). Mirrors the per-pixel body of SKY_multiple_scattering_precompute_
 * texture above; reuses the MIT-licensed SkyMultipleScattering class and the
 * static spectral_to_xyz() in this TU. SkyMultipleScattering::get_inscattering
 * is public; this struct derives from it to reach it cleanly.
 * ------------------------------------------------------------------------- */
struct SKYMultipleScatteringContext : public SkyMultipleScattering {
  float3 sun_dir;
  float altitude_km;
  SKYMultipleScatteringContext(float sun_elevation,
                               float air_density,
                               float aerosol_density,
                               float ozone_density,
                               float altitude)
      : SkyMultipleScattering(air_density, aerosol_density, ozone_density)
  {
    precompute_lut();
    altitude_km = clamp(altitude, 1.0f, 99999.0f) / 1000.0f;
    const float sun_zenith_cos_angle = cosf(M_PI_2_F - sun_elevation);
...
void *SKY_multiple_scattering_create(float sun_elevation,
                                     ...
                                     float ozone_density)
{
  return new SKYMultipleScatteringContext(
      sun_elevation, air_density, aerosol_density, ozone_density, altitude);
}

void SKY_multiple_scattering_eval_xyz(void *context, const float ray_dir[3], float r_xyz[3])
{
  const SKYMultipleScatteringContext *ctx = static_cast<SKYMultipleScatteringContext *>(context);
  const float3 dir = make_float3(ray_dir[0], ray_dir[1], ray_dir[2]);
  const float3 ray_origin = make_float3(0.0f, 0.0f, EARTH_RADIUS + ctx->altitude_km);
  const float atmos_dist = ray_sphere_intersection(ray_origin, dir, ATMOSPHERE_RADIUS);
  const float ground_dist = ray_sphere_intersection(ray_origin, dir, EARTH_RADIUS);
  const float t_d = (ground_dist < 0.0f) ? atmos_dist : ground_dist;
  const float4 L = ctx->get_inscattering(ctx->sun_dir, ray_origin, dir, t_d);
  const float3 xyz = spectral_to_xyz(L);
  r_xyz[0] = xyz.x;
  r_xyz[1] = xyz.y;
  r_xyz[2] = xyz.z;
}
...
{
  delete static_cast<SKYMultipleScatteringContext *>(context);
}
