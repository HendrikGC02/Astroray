...
/* -------------------------------------------------------------------------
 * Astroray addition (Batch J, 2026-09-13) -- NOT part of upstream Blender.
 * Thin wrapper that exposes the single-scattering 21-wavelength spectrum for a
 * view direction so a later package can feed the spectral integrator directly
 * (brief: "keep the per-wavelength intermediate reachable"). It only calls the
 * existing static single_scattering() above; no model change. `ray_dir` and
 * `sun_dir` are Z-up (z = up), matching geographical_to_direction. Kept inside
 * this Apache-2.0 translation unit because single_scattering() has internal
 * linkage.
 * ------------------------------------------------------------------------- */
...
{
  altitude = clamp(altitude, 1.0f, 59999.0f);
  const float3 cam_pos = make_float3(0.0f, 0.0f, EARTH_RADIUS + altitude);
  const float3 sun_dir = geographical_to_direction(sun_elevation, 0.0f);
  const float3 dir = make_float3(ray_dir[0], ray_dir[1], ray_dir[2]);
  single_scattering(dir, sun_dir, cam_pos, air_density, aerosol_density, ozone_density, r_spectrum);
}

/* Astroray addition (Batch J, 2026-09-13) -- NOT upstream. Single-scattering
 * sky radiance as CIE XYZ for one Z-up view direction, so the equirect bake can
 * evaluate the model per output pixel directly (exact orientation, no layout
 * resample). Reuses the static single_scattering() + spec_to_xyz() in this TU. */
