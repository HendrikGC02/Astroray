...
// can #include the same functions into a .cuh unchanged.
//
// pkg225 Stage 4: every function is __host__ __device__ under nvcc (AR_HAIR_HD)
// so gpu_hair.cuh reuses the EXACT same Mp/Np/Ap/Fresnel/sigma_a math — CPU/GPU
// parity is by construction. Only min/max/abs/swap use the device-safe global
// forms (fminf/fmaxf/fabsf / manual swap); the std:: transcendentals (sqrt/exp/
// log/pow/sinh/sin/cos/asin/atan2) are __host__ __device__ under CUDA's <cmath>.
#include <cmath>
#if defined(__CUDACC__)
#  define AR_HAIR_HD __host__ __device__
#else
#  define AR_HAIR_HD
#endif
...
static AR_HAIR_HD inline float sqr(float v) { return v * v; }
static AR_HAIR_HD inline float safeSqrt(float v) { return std::sqrt(fmaxf(0.0f, v)); }
static AR_HAIR_HD inline float safeAsin(float v) { return std::asin(fminf(1.0f, fmaxf(-1.0f, v))); }
...
static AR_HAIR_HD inline float besselI0(float x) {
...
static AR_HAIR_HD inline float logBesselI0(float x) {
    if (x > 12.0f)
...
static AR_HAIR_HD inline float Mp(float cosThetaI, float cosThetaO,
...
static AR_HAIR_HD inline float logistic(float x, float s) {
    x = fabsf(x);
    float e = std::exp(-x / s);
    return e / (s * sqr(1.0f + e));
}
static AR_HAIR_HD inline float logisticCDF(float x, float s) {
    return 1.0f / (1.0f + std::exp(-x / s));
}
static AR_HAIR_HD inline float trimmedLogistic(float x, float s, float a, float b) {
    return logistic(x, s) / (logisticCDF(b, s) - logisticCDF(a, s));
}
static AR_HAIR_HD inline float sampleTrimmedLogistic(float u, float s, float a, float b) {
...
static AR_HAIR_HD inline float deltaPhi(int p, float gammaO, float gammaT) {
    return 2.0f * (float)p * gammaT - 2.0f * gammaO + (float)p * kPi;
}

// Wrap an angle into [-pi, pi] (pbrt hair.cpp inline in Np).
static AR_HAIR_HD inline float wrapAngle(float phi) {
...
static AR_HAIR_HD inline float Np(float phi, int p, float s, float gammaO, float gammaT) {
...
static AR_HAIR_HD inline float longitudinalVariance(float betaM) {
    float t = 0.726f * betaM + 0.812f * sqr(betaM) + 3.7f * std::pow(betaM, 20.0f);
    return sqr(t);
}
static AR_HAIR_HD inline float azimuthalScale(float betaN) {
...
static AR_HAIR_HD inline float frDielectric(float cosThetaI, float etaI, float etaT) {
    cosThetaI = fminf(1.0f, fmaxf(-1.0f, cosThetaI));
    if (cosThetaI < 0.0f) { float _t = etaI; etaI = etaT; etaT = _t; cosThetaI = -cosThetaI; }
...
static AR_HAIR_HD inline void Ap(float f, float T, float ap[kPMax + 1]) {
    ap[0] = f;                          // R
    ap[1] = sqr(1.0f - f) * T;          // TT
    ap[2] = ap[1] * T * f;              // TRT
    // residual TRRT+ geometric tail (pbrt: ap[pMax] = ap[pMax-1]*f*T/(1-T*f))
    ap[3] = ap[2] * f * T / fmaxf(1e-5f, 1.0f - T * f);
}
...
static AR_HAIR_HD inline void apPdf(const float ap[kPMax + 1], float pdf[kPMax + 1]) {
    float sum = 0.0f;
...
static AR_HAIR_HD inline void tiltThetaO(const AlphaTilt& t, int p,
...
static AR_HAIR_HD inline void melaninCoeffs(int channel, float& ce, float& cp) {
