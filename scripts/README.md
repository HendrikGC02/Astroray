...
| Gate (b) weighted socket-coverage scorer (pkg278; `--collect` in Blender computes scene-used reachability and active constant/link variants, `--freeze` writes immutable `docs/blender_parity/coverage_input_v2.json` with an asset- and ledger-locked population, `--score` re-verifies the committed input, assets, snapshot and ledger before separately scoring CPU/GPU; #823-gated, never green before it lands) | `benchmarks/reference_corpus/coverage_report.py` (`--collect` run inside Blender; `--freeze`/`--score`/`--self-test` in the repo's normal Python env) |
