...
| Gate (b) weighted socket-coverage scorer (pkg278; `--collect` in Blender computes reachability backward from active Material/World/Light Output inputs, `--freeze` writes the versioned `docs/blender_parity/coverage_input_v1.json` input manifest, `--score` re-verifies it against disk + the snapshot and scores CPU/GPU separately; #823-gated, never green before it lands) | `benchmarks/reference_corpus/coverage_report.py` (`--collect` run inside Blender; `--freeze`/`--score`/`--self-test` in the repo's normal Python env) |
