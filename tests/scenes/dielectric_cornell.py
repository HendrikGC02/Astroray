"""pkg55 Phase B' Session 4 — Dielectric + Metal + Lambertian Cornell box.

Test scene for the CPU wavefront Session 4 expansion. Extends the
metal_cornell base with a dielectric sphere, adding glass BSDF
to the growing-oracle feature surface.

Scope: Lambertian BSDFs + metal (GGX conductor) + dielectric (glass) + emissive area light.
No Disney, no env map, no thin_glass.

The reference PTs and the CPU wavefront driver now cover
{lambertian, metal, dielectric, diffuse_light}. This scene validates that Session 4's
dielectric support achieves bit-identity CPU↔CPU.

Layout: classic Cornell box — 6 walls + 1 Lambertian sphere + 1 metal
sphere + 1 dielectric sphere + 1 ceiling area light. White walls, red left, green right,
neutral floor/ceiling.
"""


def build_scene(renderer):
    """Populate *renderer* with the Dielectric + Metal + Lambertian Cornell scene.
    ...
    return dict(white=white_id, red=red_id, green=green_id,
                ...
                dielectric_sphere=dielectric_sphere_id,
                light=light_id)
