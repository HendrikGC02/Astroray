"""pkg55 Phase B' Session 6 — Thin Glass + Disney + Dielectric + Metal + Lambertian Cornell box.

Test scene for the CPU wavefront Session 6 expansion. Extends the
disney_cornell base with a thin_glass sphere, adding thin_glass BSDF to the
growing-oracle feature surface.

Scope: Lambertian BSDFs + metal (GGX conductor) + dielectric (glass) + disney
       + thin_glass + emissive area light.
...
validates that Session 6's thin_glass support achieves bit-identity CPU↔CPU.

Layout: classic Cornell box — 6 walls + 1 Lambertian sphere + 1 metal
sphere + 1 dielectric sphere + 1 disney sphere + 1 thin_glass sphere + 1 ceiling area light.
White walls, red left, green right, neutral floor/ceiling.
"""


def build_scene(renderer):
    """Populate *renderer* with the Thin Glass + Disney + Dielectric + Metal + Lambertian Cornell scene.
    ...
    return dict(white=white_id, red=red_id, green=green_id,
                ...
                thin_glass_sphere=thin_glass_sphere_id, light=light_id)
