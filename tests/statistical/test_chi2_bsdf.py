...
@pytest.mark.xfail(
    strict=True,
    reason="chi2[0.3-45] is red at 34987.970271 (1025 dof, significance "
    "0.002008), BUT this magnitude is ~90% an ires=4 QUADRATURE ARTIFACT, not "
    "a sampler defect (pkg150 measurement 2026-08-02, "
    ".astroray_plan/packages/pkg150-disney-dielectric-vndf-hemisphere-masking.md). Raising "
    "ONLY the harness quadrature ires 4->8->16 drops raw chi2 "
    "35107->3942->3240 and the pdf integral 0.967->0.999->0.999 for the "
    "IDENTICAL sampler/config: the coarse ires=4 trapezoid under-integrates "
    "the peaked microfacet lobe near the equator (same class the full-grid "
    "test documents as grid-limited). The true (ires=8) residual ~3942 splits "
    "~50/50 reflection(1981)/transmission(1961) across all cos-theta bands -- "
    "a small symmetric effect in BOTH lobes, NOT a reflection-specific delta "
    "spike. pkg150's former hypothesis (residual == sample()'s VNDF-reflection "
    "candidate rejected to a smooth-mirror-delta) is DISPROVEN: implementing "
    "the PBRT-v4 dead-sample fix (pdf=0 on SameHemisphere failure) removed "
    "that fallback entirely (delta fallback 0.16%->0.00% at [0.3-45]) yet "
    "moved chi2 only 2.4% (34987.97->34141.83), and it REGRESSES the "
    "high-roughness furnace (r=1.0 0.997->0.788 CPU; the below-horizon dead "
    "fraction scales 0.08%@r0.3 -> 7.1%@r1.0 and compounds over the depth-32 "
    "integral -- the #517 trap). pkg150's charter (reflection sampleable at "
    "grazing, sample()/pdf() describe the same set) was ALREADY met on main by "
    "the pkg149 sampler fix: reflection acceptance at [0.3-45] is 5.1% with "
    "sample-pdf median rel err 0.0000 (N=300k). Closing this gate for real "
    "needs reflection-lobe multi-scatter compensation (Kulla-Conty/Turquin -- "
    "separately-citable physics, routed to its own spec), NOT a "
    "coverage/dead-sample tweak. Full trail: "
    ".astroray_plan/packages/pkg150-disney-dielectric-vndf-hemisphere-masking.md, "
    ".astroray_plan/docs/pkg149-disney-rough-transmission-research.md, "
    ".astroray_plan/docs/pkg154-furnace-deficit-findings.md. Do NOT delete or "
    "widen; do not re-attribute to pkg150 (charter resolved-by-pkg149).")
@pytest.mark.parametrize("theta_deg", [45])
