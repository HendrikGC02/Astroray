...
@pytest.mark.xfail(
    ...
    "fixed): at THIS specific config (roughness=0.3, theta=45), sample()'s "
    "VNDF-reflection candidate is rejected before eval() is ever reached -- "
    ...
    "multiscatter term first). Root cause #3 (NEWLY measured, out of pkg138 "
    "scope per spec's explicit Non-goal 'the transmission/refraction lobe -- "
    "roughTransmissionEval/Pdf are correct and untouched'): even independent "
    "of #2, roughTransmissionPdf's analytic peak (152 deg from normal, "
    "exactly the smooth Snell angle for theta_o=45/ior=1.5) does NOT match "
    "where sample()'s VNDF-then-refract draws actually land (measured peak "
    "168-170 deg, restricted to the exact incidence plane) -- a genuine "
    "~16-18 deg sample/pdf shape mismatch in the transmission lobe, which "
    "carries ~92-96% of this material's sampled weight at this Fresnel and "
    "appears to be the dominant chi2 contributor. Neither #2 nor #3 was "
    "anticipated by the pkg138 spec's root-cause diagnosis. Full measurements, "
    ...
    "Follow-up needed: a dedicated masking/multiscatter-compensation pass "
    "(#2) and a dedicated rough-transmission VNDF sample/pdf re-derivation "
    "(#3) before this gate can go green. Do NOT delete or widen -- documents "
    "two real, further defects.")
@pytest.mark.parametrize("theta_deg", [45])
