...
@pytest.mark.xfail(
    strict=True,
    reason="pkg149 (2026-07-25, stacked on pkg151/PR #519 + pkg154's frontFace/"
    "clamp fixes) CLOSED its owned root cause -- chi2 is measured UNCHANGED at "
    "34987.970271 (bit-identical to the pre-pkg154-patch pkg149 measurement), "
    "which is the EXPECTED result: this is a sample()/pdf() SELF-CONSISTENCY "
    "test, and pkg149's azimuth fix (sampleGgxVNDF's swapped Lerp args, "
    "pbrt-v4 scattering.h/math.h) moved both functions together (peak "
    "alignment 15.7deg -> 0.45deg, gate <2deg, N=180977 -- "
    "test_disney_transmission_peak_alignment.py), so their RELATIVE "
    "agreement, which is what chi2 measures, is unaffected by that fix or by "
    "pkg154's eval()-magnitude-only fixes (frontFace-based etap selection, "
    "clamp(0,4) removal -- .astroray_plan/docs/pkg154-furnace-deficit-"
    "findings.md; furnace 0.997-0.999 across roughness {0.05,0.1,0.3,0.6,1.0}, "
    "gate [0.92,1.03]). Remaining residual chi2 (34987.970271, still red at "
    "significance 0.002008/1025 dof) is OWNED SOLELY BY pkg150 "
    "'disney-dielectric-vndf-hemisphere-masking': sample()'s VNDF-reflection "
    "candidate is rejected before eval() is ever reached (Fresnel roulette "
    "systematically selects small-HdotO half-vectors whose reflection fails "
    "the same-hemisphere check, falling back to the smooth-mirror delta -- "
    "measured N>=100000, roughness in {0.3,0.5}, theta in {0,30,45}); a "
    "PBRT-v4-faithful fix (dead/pdf=0 sample on SameHemisphere failure) was "
    "tried and regresses the furnace to ~0.0 without a compensating Smith "
    "joint-G(wo,wi)/multiscatter term first. Full trail: "
    ".astroray_plan/docs/pkg149-disney-rough-transmission-research.md, "
    ".astroray_plan/docs/pkg154-furnace-deficit-findings.md. Do NOT delete "
    "or widen -- this gate's un-xfail obligation now rests entirely with "
    "pkg150; no other package may close while it is xfail.")
@pytest.mark.parametrize("theta_deg", [45])
