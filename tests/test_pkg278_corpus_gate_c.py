...
def test_fake_six_leg_capture_freezes_before_spawn_and_keeps_npy_png(tmp_path, monkeypatch):
    ...
    def fake_leg(_, args, __, ___):
        ...
        report = {"corpus_scene": scene, "blend_sha256": roles[next(k for k,v in roles.items() if v["scene_id"] == scene)]["scene_sha256"], "freeze_sha256": args[args.index("--gate-c-freeze-sha256") + 1], "requested_device": device, "effective_device": device, "build_id": "b1", "module_path": "C:/candidate/astroray.pyd", "module_sha256": "a" * 64, "addon_path": "C:/candidate/addon/__init__.py", "addon_sha256": "b" * 64, "telemetry": [{"device": -1 if device == "cpu" else 0}], "engine": "CUSTOM_RAYTRACER", "res_x": 16, "res_y": 16, "samples": 4, "mutation_receipt": {"kind": args[args.index("--gate-c-control") + 1], "ok": True} if "--gate-c-control" in args else {"kind": "baseline", "ok": True}}
