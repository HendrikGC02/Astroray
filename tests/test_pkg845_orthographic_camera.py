...
G = SL.ORTHO_GRID
PERSP_LENS = 50.0  # PERSP control: same grid, camera and shift, 50 mm / 36 mm AUTO fit
FRAMES = {"landscape": "landscape", "portrait": "portrait",
          "persp_landscape": "landscape", "persp_portrait": "portrait"}
...
def _write_reference():
    ...
    hw = G["line_w"] / 2.0
    for name, orient in FRAMES.items():
        W, H = G[orient]
        scene, *_ = SL.build_camera_lens_ortho_scene(bpy, (W, H))
        cam = scene.camera
        if name.startswith("persp_"):
            cam.data.type, cam.data.lens, cam.data.sensor_width = "PERSP", PERSP_LENS, 36.0
...
def _ortho_cam_obj(cam_type="ORTHO"):
    ...
    data = types.SimpleNamespace(type=cam_type, ortho_scale=G["ortho_scale"],
                                 sensor_fit="AUTO", shift_x=G["shift"][0],
                                 shift_y=G["shift"][1], clip_start=G["clip"][0],
                                 clip_end=G["clip"][1], dof=dof, lens=PERSP_LENS,
