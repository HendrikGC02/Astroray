"""pkg55 Phase B' Session 8 — Closure Graph (closure_matte) Cornell CPU wavefront bit-identity gate.

Session 8 extends the shared kernel to support closure_graph materials
(closure_matte — closure-graph-based diffuse).
This test validates EXACT bit-identity CPU wavefront ↔ reference_pt_wavefront
on the closure_matte + diffuse_light + thin_glass + disney + dielectric + metal
+ Lambertian Cornell scene at 1 spp — zero mismatched fields across the full
...
Sessions 2c/3/4/5/6/7. The mechanism is unchanged; the scope has grown to include
closure_graph (materials using the closure-graph mechanism, such as closure_matte).

Spec: .astroray_plan/packages/pkg55-wavefront-soa-refactor.md §"Session 8".
...
import closure_graph_cornell


def _build_renderer(width, height, seed, max_depth):
    r = astroray.Renderer()
    closure_graph_cornell.build_scene(r)
    closure_graph_cornell.setup_camera(r, width=width, height=height)
    r.set_seed(seed)
...
def test_cpu_wavefront_closure_graph_bit_identity():
    """EXACT bit-identity gate: CPU wavefront == reference_pt_wavefront, 1 spp.

    Session 8 close gate: snapshot-stream equality for the closure_matte +
    diffuse_light + thin_glass + disney + dielectric + metal + Lambertian
    Cornell scene. max_abs_diff must be EXACTLY 0.0 and
    total_diverging_fields EXACTLY 0.
    """
    WIDTH, HEIGHT = 16, 16
    SEED = 424242
    SPP = 1
    ...
    # Session 8 close gate — EXACT 0.0 / exact-int. Bit-identity is by
    ...
    print(f"\n[pkg55-Session8 bit-identity gate] PASS: snapshot stream is "
          f"EXACTLY bit-identical (max diff = {max_abs_diff!r}, "
          f"diverging fields = {total_diverging_fields}) — by construction.")


def test_cpu_wavefront_closure_graph_determinism():
    ...
    print("\n[pkg55-Session8 determinism] PASS: same-seed wavefront renders "
