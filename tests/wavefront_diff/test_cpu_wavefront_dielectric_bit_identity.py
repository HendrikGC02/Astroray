"""pkg55 Phase B' Session 4 — Dielectric Cornell CPU wavefront bit-identity gate.

Session 4 extends the shared kernel to support dielectric (glass) materials.
This test validates EXACT bit-identity CPU wavefront ↔ reference_pt_wavefront
on the dielectric + metal + Lambertian Cornell scene at 1 spp — zero mismatched
...
Session 2c/3. The mechanism is unchanged; the scope has grown to include dielectric.

Spec: .astroray_plan/packages/pkg55-wavefront-soa-refactor.md §"Session 4".
...
import dielectric_cornell


def _build_renderer(width, height, seed, max_depth):
    r = astroray.Renderer()
    dielectric_cornell.build_scene(r)
    dielectric_cornell.setup_camera(r, width=width, height=height)
    r.set_seed(seed)
...
def test_cpu_wavefront_dielectric_bit_identity():
    """EXACT bit-identity gate: CPU wavefront == reference_pt_wavefront, 1 spp.

    Session 4 close gate: snapshot-stream equality for the dielectric + metal +
    Lambertian Cornell scene. max_abs_diff must be EXACTLY 0.0 and
    ...
    # Session 4 close gate — EXACT 0.0 / exact-int. Bit-identity is by
...
def test_cpu_wavefront_dielectric_determinism():
    ...
    print("\n[pkg55-Session4 determinism] PASS: same-seed wavefront renders "
