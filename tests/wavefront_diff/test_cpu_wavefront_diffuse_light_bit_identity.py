"""pkg55 Phase B' Session 7 — Diffuse Light Cornell CPU wavefront bit-identity gate.

Session 7 extends the shared kernel to support diffuse_light materials (pure
emissive geometry, distinct from area light triangles).
This test validates EXACT bit-identity CPU wavefront ↔ reference_pt_wavefront
on the diffuse_light + thin_glass + disney + dielectric + metal + Lambertian
Cornell scene at 1 spp — zero mismatched fields across the full snapshot stream,
max abs diff EXACTLY 0.0 for floats, exact for ints, at all 5 stage boundaries.

Bit-identity is BY CONSTRUCTION (shared kernel + carried live state), same as
Sessions 2c/3/4/5/6. The mechanism is unchanged; the scope has grown to include
diffuse_light (emissive sphere that terminates paths on hit).

Spec: .astroray_plan/packages/pkg55-wavefront-soa-refactor.md §"Session 7".
...
import diffuse_light_cornell


def _build_renderer(width, height, seed, max_depth):
    r = astroray.Renderer()
    diffuse_light_cornell.build_scene(r)
    diffuse_light_cornell.setup_camera(r, width=width, height=height)
    r.set_seed(seed)
...
def test_cpu_wavefront_diffuse_light_bit_identity():
    """EXACT bit-identity gate: CPU wavefront == reference_pt_wavefront, 1 spp.

    Session 7 close gate: snapshot-stream equality for the diffuse_light +
    thin_glass + disney + dielectric + metal + Lambertian Cornell scene.
    ...
    # Session 7 close gate — EXACT 0.0 / exact-int. Bit-identity is by
    ...
    print(f"\n[pkg55-Session7 bit-identity gate] PASS: snapshot stream is "
          f"EXACTLY bit-identical (max diff = {max_abs_diff!r}, "
          f"diverging fields = {total_diverging_fields}) — by construction.")


def test_cpu_wavefront_diffuse_light_determinism():
    ...
    print("\n[pkg55-Session7 determinism] PASS: same-seed wavefront renders "
