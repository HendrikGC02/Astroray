"""pkg55 Phase B' Session 5 — Disney Cornell CPU wavefront bit-identity gate.

Session 5 extends the shared kernel to support disney (principled BSDF) materials.
This test validates EXACT bit-identity CPU wavefront ↔ reference_pt_wavefront
on the disney + dielectric + metal + Lambertian Cornell scene at 1 spp — zero mismatched fields
across the full snapshot stream, max abs diff EXACTLY 0.0 for floats, exact
for ints, at all 5 stage boundaries.

Bit-identity is BY CONSTRUCTION (shared kernel + carried live state), same as
Sessions 2c/3/4. The mechanism is unchanged; the scope has grown to include disney.

Spec: .astroray_plan/packages/pkg55-wavefront-soa-refactor.md §"Session 5".
...
import disney_cornell


def _build_renderer(width, height, seed, max_depth):
    r = astroray.Renderer()
    disney_cornell.build_scene(r)
    disney_cornell.setup_camera(r, width=width, height=height)
    r.set_seed(seed)
...
def test_cpu_wavefront_disney_bit_identity():
    """EXACT bit-identity gate: CPU wavefront == reference_pt_wavefront, 1 spp.

    Session 5 close gate: snapshot-stream equality for the disney + dielectric +
    metal + Lambertian Cornell scene. max_abs_diff must be EXACTLY 0.0 and
    ...
    # Session 5 close gate — EXACT 0.0 / exact-int. Bit-identity is by
...
def test_cpu_wavefront_disney_determinism():
    ...
    print("\n[pkg55-Session5 determinism] PASS: same-seed wavefront renders "
