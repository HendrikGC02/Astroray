"""pkg55 Phase B' Session 3 — Metal Cornell CPU wavefront bit-identity gate.

Session 3 extends the shared kernel to support metal (GGX conductor) materials.
This test validates EXACT bit-identity CPU wavefront ↔ reference_pt_wavefront
on the metal + Lambertian Cornell scene at 1 spp — zero mismatched fields
...
Session 2c. The mechanism is unchanged; the scope has grown to include metal.

Spec: .astroray_plan/packages/pkg55-wavefront-soa-refactor.md §"Session 3".
...
import metal_cornell


def _build_renderer(width, height, seed, max_depth):
    r = astroray.Renderer()
    metal_cornell.build_scene(r)
    metal_cornell.setup_camera(r, width=width, height=height)
    r.set_seed(seed)
...
def test_cpu_wavefront_metal_bit_identity():
    """EXACT bit-identity gate: CPU wavefront == reference_pt_wavefront, 1 spp.

    Session 3 close gate: snapshot-stream equality for the metal + Lambertian
    Cornell scene. max_abs_diff must be EXACTLY 0.0 and total_diverging_fields
    EXACTLY 0.
    """
    WIDTH, HEIGHT = 16, 16
    SEED = 424242
    ...
    # Session 3 close gate — EXACT 0.0 / exact-int. Bit-identity is by
...
def test_cpu_wavefront_metal_determinism():
    ...
    print("\n[pkg55-Session3 determinism] PASS: same-seed wavefront renders "
