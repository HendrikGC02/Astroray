"""pkg55 Phase B' Session N+1 — env-map miss + complete pipeline bit-identity gate.

Session N+1 extends the shared kernel to handle environment-map misses (env map /
backgroundColor / default sky gradient) when a ray misses all geometry. This
completes the CPU wavefront pipeline — shadow ray, miss, Russian roulette, and
accumulate were already present in Sessions 2c-8; this session just fills the
env-map miss gap.

This test validates EXACT bit-identity CPU wavefront ↔ reference_pt_wavefront
on the mixed-7-material + env-map Cornell scene at 1 spp — zero mismatched
fields across the full snapshot stream, max abs diff EXACTLY 0.0 for floats,
exact for ints, at all 5 stage boundaries.

Bit-identity is BY CONSTRUCTION (shared kernel + carried live state), same as
Sessions 2c-8. The mechanism is unchanged; the scope has grown to include
env-map misses.
...
def test_cpu_wavefront_session_n1_bit_identity():
    """EXACT bit-identity gate: CPU wavefront == reference_pt_wavefront, 1 spp.

    Session N+1 close gate: snapshot-stream equality for the mixed-7-material
    + env-map Cornell scene. max_abs_diff must be EXACTLY 0.0 and
    ...
    # Session N+1 close gate — EXACT 0.0 / exact-int. Bit-identity is by
...
def test_cpu_wavefront_session_n1_determinism():
    ...
    print("\n[pkg55-SessionN+1 determinism] PASS: same-seed wavefront renders "
